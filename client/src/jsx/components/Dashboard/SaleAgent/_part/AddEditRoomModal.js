...
import { createRoom, createUser, updateUser } from "../../../../../store/actions/User";
...
const AddUserModal = ({ onClick, active, data }) => {
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    if (values.email && values.password && values.first_name && (values.file || values.profile)) {
      ...
      dispatch((isNew ? createUser : updateUser)(formData, onClick, refreshState));
    } else {
